    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let operation = match parts.next() {
            Some("turn") => match parts.next() {
                ...
                _ => return Err(Error::new(std::io::ErrorKind::InvalidInput, "Invalid operation"))
            },
        ...
        let (xstart, ystart) = match parts.next() {
            ...
            _ => return Err(Error::new(std::io::ErrorKind::InvalidInput, "Invalid starts"))
        };

        if parts.next() != Some("through") {
            return Err(Error::new(std::io::ErrorKind::InvalidInput, "Missing 'through' keyword"))
        }
